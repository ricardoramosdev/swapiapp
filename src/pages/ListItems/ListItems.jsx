...
import { Button, Card, Form, Input, List, Modal, Pagination } from 'antd'
  ...
  <div className="search">
    ...
    renderItem={item => (
      <List.Item>
            <Card id={item.id}title={item.name} actions={[
               ...
               <Button onClick={()=>moreInfo(item, listIt.indexOf(item))}><SettingOutlined key="setting" /></Button>,
    ...
    />
     <Modal
      ...
      footer={[<Button hidden={edit}>Editar</Button>,
      <Button onClick={()=>{setVisible(false)}}>Cancel</Button>,
      <Button onClick={()=>{setVisible(false)}} hidden={!edit}>Ok</Button>
    ]}
      >
        <Button onClick={()=>editPerson(selectedPerson, listIt.indexOf(selectedPerson))}><EditOutlined key="edit" /></Button>
        <div>{selectedPerson.name}</div>
        <div className="info">
        <Form className="info-list"
        disabled={edit}
        >

          <Form.Item>
            <label for='peliculas'>Peliculas: </label>
            <Input name='peliculas' id='peliculas'value={selectedPerson.films?.map(el=><li>{el}</li>)}/>{selectedPerson.films?.map(el=><li>{el}</li>)}
            </Form.Item>

          <Form.Item>
            <label for='homeworld'>Planeta Hogar: </label>
            <Input name='homeworld' id='homeworld' value={selectedPerson.homeworld}/> </Form.Item>

          <Form.Item>
          <label htmlFor="starships">Naves: </label>
          <Input name='starships'id='starships'/>{selectedPerson.starships?.map(el=><li>{el}</li>)}</Form.Item>

          <Form.Item>
            <label htmlFor='vehicles'>Vehiculos:</label>
            <Input name='vehicles' id='vehicles' value={selectedPerson.vehicles?.map(el=><li>{el}</li>)}/>
             </Form.Item>
          ...
          <Form.Item>
            ...
            <Input name='gender' id='gender' value={selectedPerson.gender}/></Form.Item>
        </Form>
        </div>
      </Modal>
